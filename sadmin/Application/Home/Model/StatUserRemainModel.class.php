...
class StatUserRemainModel extends Model
{
    /**
     * 取得用户留存统计数据
     * @author liyao
     */
    public function queryStatUserRemainData($where, $pageParam, $field = "*")
    {
        $ret = array(
        ...
        try {
            ...
            set_exception(__FILE__, __LINE__, "[queryStatUserRemainData] select failed: ".$e->getMessage());
     ...
     * @author liyao
     */
    public function queryStatUserRemainChartData($where, $chartMap, $stime, $etime)
    {
        $ret = array(
            'code' => ERRCODE_SUCCESS,
            'msg' => "",
        ...
        try {
            ...
            foreach ($chartMap as $k => $v) {
                ...
                $chartData[] = ['name' => $v, 'key' => $k, 'xAxis' => $xVal, 'data' => $da];
            }
        } catch(\Exception $e) {
            set_exception(__FILE__, __LINE__, "[queryStatUserRemainChartData] select failed: ".$e->getMessage());
            $ret['code'] = ERRCODE_DB_SELECT_ERR;
            $ret['msg'] = $e->getMessage();
            return $ret;
        }
     ...
     * @author liyao
     */
    public function queryStatUserRemainAllData($where, $field = "*")
    {
        $ret = array(
            'code' => ERRCODE_SUCCESS,
            'msg' => "",
        ...
        try {
            ...
            set_exception(__FILE__, __LINE__, "[queryStatUserRemainAllData] select failed: ".$e->getMessage());
            $ret['code'] = ERRCODE_DB_SELECT_ERR;
            $ret['msg'] = $e->getMessage();
            return $ret;
        }
     ...
     * @author liyao
     */
    public function insertStatUserRemain($data, $gameid, $curDate, $insertf = true)
    {
        try {
            ...
            set_exception(__FILE__, __LINE__, "[insertStatUserRemain] insert failed ".$e->getMessage());
        }
    }
}
