...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __check_login()
    {
        ...
        if ($this->uri->segment(1) == 'admin' || $this->uri->segment(1) == 'api') {
    ...
    //失败返回
    public function error_response($code,$data=[])
    {
        if(isset($this->codes[$code])){
            echo json_encode(['success'=>false,'code'=>$code,'msg'=>$this->codes[$code],'data'=>$data]);
        } else {
            echo json_encode(['success'=>false,'code'=>$code,'msg'=>'未知错误','data'=>$data]);
        }
        exit;
    }

    public function success_response($data,$code=200)
    {
        if(isset($this->codes[$code])){
            echo json_encode(['success'=>true,'code'=>$code,'msg'=>$this->codes[$code],'data'=>$data]);
        } else {
            echo json_encode(['success'=>true,'code'=>$code,'msg'=>'请求成功','data'=>$data]);
        }
        exit;
